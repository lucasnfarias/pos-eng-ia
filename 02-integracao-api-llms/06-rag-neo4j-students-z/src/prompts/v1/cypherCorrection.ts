...
export const getSystemPrompt = (schema: string): string => {
  return JSON.stringify({
    ...
    example: {
      failed:
        "WITH SUM(p.amount) AS total ... RETURN c.name, revenue, round(revenue / total, 2)",
      error: "Implicit grouping expressions: totalRevenue",
      corrected:
        "WITH SUM(p.amount) AS total ... WITH total, c.name AS name, SUM(p2.amount) AS rev RETURN name, rev, round(rev / total, 2)",
