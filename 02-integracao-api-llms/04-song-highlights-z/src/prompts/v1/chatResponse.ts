...
export const UserPreferencesSchema = z.object({
  ...
  favoriteGenres: z
    .array(z.string())
    .optional()
    .describe("Gêneros musicais favoritos"),
  favoriteBands: z
    .array(z.string())
    .optional()
    .describe("Bandas ou artistas favoritos"),
  ...
  additionalInfo: z
    .string()
    .optional()
    .describe("Outras preferências relevantes mencionadas"),
});

export const ChatResponseSchema = z.object({
  message: z.string().describe("A resposta conversacional para o usuário"),
  preferences: UserPreferencesSchema.optional().describe(
    "Preferências extraídas desta mensagem",
  ),
  shouldSavePreferences: z
    .boolean()
    .describe("Se as preferências extraídas devem ser salvas"),
});
...
export const getSystemPrompt = (userContext?: string) => {
  return JSON.stringify({
    ...
    regras_de_extracao: {
      ...
      nao_extrair:
        "Saudações simples, perguntas sem novas informações, reações genéricas sem conteúdo novo",
    },
